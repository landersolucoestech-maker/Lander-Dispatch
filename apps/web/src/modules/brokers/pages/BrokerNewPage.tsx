...
import { z } from "zod";
import { useCreateBroker } from "@workspace/api-client-react";
import { useLocation } from "wouter";
import { useQueryClient } from "@tanstack/react-query";
import { getListBrokersQueryKey } from "@workspace/api-client-react";
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 p-8 overflow-y-auto max-w-4xl mx-auto w-full">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight uppercase">New Broker</h1>
          <p className="text-sm font-mono text-muted-foreground">Onboard a new broker partner</p>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="dispatch@broker.com" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-mono text-muted-foreground uppercase">Terms & Status</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                control={form.control}
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={createBroker.isPending} className="gap-2">
              <Save className="w-4 h-4" />
              {createBroker.isPending ? "Saving..." : "Save Broker"}
            </Button>
          </div>
        </form>
      </Form>
