...
const MODULE_ROUTES = [
  ...
  { match: "/agenda", module: "agenda" },
  { match: "/chat", module: "chat" },
  { match: "/tasks", module: "tasks" },
  { match: "/marketing", module: "marketing" },
...
const HEADER_COPY: Record<string, { title: string; description: string }> = {
  ...
  chat: { title: "CHAT", description: "Internal conversations and centralized support operations." },
  tasks: { title: "TASK", description: "Operational tasks, owners, priorities, deadlines and checklists." },
  marketing: { title: "MARKETING", description: "Overview, briefing, calendar, campaigns, AI creative, metrics and tasks." },
};

function getPageLabel(location: string) {
  const allItems = NAV_ITEMS.flatMap((item) => [{ href: item.href, label: item.label }, ...(item.sub ?? [])]);
  return allItems.sort((left, right) => right.href.length - left.href.length).find((item) => location.startsWith(item.href))?.label ?? "Lander Dispatch";
}
...
function HeaderActions({ location, crmTab }: { location: string; crmTab: CrmTab }) {
  ...
  if (location === "/loads") return <div className="flex items-center gap-2"><button type="button" className={secondaryClass} onClick={() => dispatch("lander:loads-import-pdf")}><FileText className="h-4 w-4" />Import PDF</button><button type="button" className={primaryClass} onClick={() => dispatch("lander:loads-create")}><Plus className="h-4 w-4" />Create Load</button></div>;
  if (location === "/crm") return crmTab === "leads" ? <button type="button" className={primaryClass} onClick={() => dispatch("lander:crm-create-lead")}><Plus className="h-4 w-4" />Create Lead</button> : <button type="button" className={primaryClass} onClick={() => dispatch("lander:crm-create-contact")}><UserPlus className="h-4 w-4" />Create Contact</button>;
  if (location === "/carriers") return <button type="button" className={primaryClass} onClick={() => dispatch("lander:carriers-add")}><Plus className="h-4 w-4" />Add Carrier</button>;
  if (location === "/brokers") return <button type="button" className={primaryClass} onClick={() => dispatch("lander:brokers-add")}><Plus className="h-4 w-4" />Add Broker</button>;
  if (location === "/documents") return <button type="button" className={primaryClass} onClick={() => dispatch("lander:documents-upload")}><Upload className="h-4 w-4" />Upload Document</button>;
  if (location === "/accounting/invoices") return <button type="button" className={primaryClass} onClick={() => dispatch("lander:invoices-create")}><Plus className="h-4 w-4" />Create Invoice</button>;
  if (location === "/accounting/transactions") return <button type="button" className={primaryClass} onClick={() => dispatch("lander:transactions-add")}><Plus className="h-4 w-4" />Add Transaction</button>;
  return null;
}
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const module = useMemo(() => MODULE_ROUTES.find((item) => location === item.match)?.module ?? "default", [location]);
  ...
  useEffect(() => {
    if (HEADER_COPY[module]) { setHeaderCopy(HEADER_COPY[module]); return; }
    const fallback = { title: getPageLabel(location), description: module === "default" ? "Smart dispatch. Stronger miles." : "" };
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      if (title) setHeaderCopy({ title, description: description ?? "" });
    });
    return () => window.cancelAnimationFrame(frame);
  }, [location, module]);

  useEffect(() => {
    if (location !== "/crm") { setCrmTab("contacts"); return; }
