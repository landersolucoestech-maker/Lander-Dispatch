...
function getOrigin(_req: Request): string {
  // Use REPLIT_DOMAINS env var — this is the registered domain for Replit OIDC.
  // Falling back to header-based origin causes invalid_redirect_uri errors.
  const domain = process.env.REPLIT_DOMAINS;
  if (domain) return `https://${domain}`;
  const proto = _req.headers['x-forwarded-proto'] || 'https';
  const host =
    _req.headers['x-forwarded-host'] || _req.headers['host'] || 'localhost';
  return `${proto}://${host}`;
}
